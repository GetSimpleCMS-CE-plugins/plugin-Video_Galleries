...
register_plugin(
    ...
    'Create and manage video galleries with shortcode or PHP function support',
    'plugins',
    ...
    <div class="vg-instructions">
        ...
        <div class="vg-instructions-grid">
            ...
            <div class="vg-instruction-item">
      		...
      		<p  style="margin:20px 0;padding:15px;border:solid 1px #ddd;background:#fafafa;border-radius:5px;">Made with 
        ...
        <?php else: ?>
            ...
            function addVideoField() {
                ...
                var videoNum = videoIndex + 1;
                var videoUrlId = 'video_url_' + videoIndex;
                var videoThumbId = 'video_thumb_' + videoIndex;
                ...
                div.innerHTML = 
                    ...
                    '<div class="vg-form-group">' +
                        ...
                        '<div class="vg-input-group">' +
                            '<input type="url" name="video_url[]" id="' + videoUrlId + '" placeholder="https://example.com/video.mp4" required>' +
                            '<button type="button" onclick="openFileBrowser(\'' + videoUrlId + '\')" class="vg-btn-browse">Browse Files</button>' +
                        '</div>' +
                    ...
                    '<div class="vg-form-group">' +
                        ...
                        '<div class="vg-input-group">' +
                            '<input type="url" name="video_thumb[]" id="' + videoThumbId + '" placeholder="https://example.com/thumbnail.jpg">' +
                            '<button type="button" onclick="openFileBrowser(\'' + videoThumbId + '\')" class="vg-btn-browse">Browse Files</button>' +
                        '</div>' +
                        '<span class="vg-hint">Image shown before video plays (recommended 16:9 aspect ratio)</span>' +
                    '</div>';
        ...
        <div class="vg-form-group">
            ...
            <div class="vg-input-group">
                <input type="url" name="video_url[]" id="video_url_<?php echo $index; ?>"
                       value="<?php echo $data ? htmlspecialchars($data['url']) : ''; ?>" 
    ...
    try {
        ...
        if (isset($_POST['video_title']) && is_array($_POST['video_title'])) {
            ...
            foreach ($_POST['video_title'] as $idx => $title) {
                if (!empty($title) && !empty($_POST['video_url'][$idx])) {
                    $stmt->execute([
                        $gallery_id,
                        strip_tags($title),
                        strip_tags($_POST['video_url'][$idx]),
                        isset($_POST['video_thumb'][$idx]) ? strip_tags($_POST['video_thumb'][$idx]) : '',
                        $idx
                    ]);
                }
            }
...
function video_galleries_display($id) {
        ...
        <div class="video-item" onclick="openVideoLightbox<?php echo $galleryId; ?>(<?php echo $idx; ?>)">
            <div class="video-wrapper">
                <?php if (!empty($video['thumb'])): ?>
                    <img src="<?php echo htmlspecialchars($video['thumb']); ?>" alt="<?php echo htmlspecialchars($video['title']); ?>">
                <?php else: ?>
    ...
    <div id="videoLightbox<?php echo $galleryId; ?>" class="video-lightbox" onclick="closeVideoLightbox<?php echo $galleryId; ?>(event)">
        <div class="lightbox-content">
            ...
            <div class="lightbox-video-container">
                <video id="lightboxVideo<?php echo $galleryId; ?>" controls>
    ...
    .video-gallery[data-columns="2"] { grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); }
    .video-gallery[data-columns="3"] { grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); }
    .video-gallery[data-columns="4"] { grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); }
    ...
    var videoGallery<?php echo $galleryId; ?> = <?php echo json_encode($gallery['videos']); ?>;
    ...
    function openVideoLightbox<?php echo $galleryId; ?>(index) {
        ...
        var videoElement = document.getElementById('lightboxVideo<?php echo $galleryId; ?>');
        var sourceElement = document.getElementById('lightboxSource<?php echo $galleryId; ?>');
        var titleElement = document.getElementById('lightboxTitle<?php echo $galleryId; ?>');
        
        // Set video source and title
        sourceElement.src = video.url;
        titleElement.textContent = video.title;
        
        // Reload video and show lightbox
        videoElement.load();
    ...
    function closeVideoLightbox<?php echo $galleryId; ?>(event) {
        // Close only if clicking outside video or on close button
